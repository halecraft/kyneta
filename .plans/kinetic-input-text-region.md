...
2. **`setRangeText("preserve")` eliminates cursor arithmetic.** Previous approaches (including `hooks-core`) compute cursor adjustment from deltas manually. `setRangeText` with `selectMode: "preserve"` makes the browser do this automatically — insert before cursor shifts it right, delete before cursor shifts it left. This eliminates ~50 lines of cursor math and the associated edge cases.

3. **The read and write directions are genuinely independent.** The read direction (delta-aware subscription) is a compiler optimization that falls out of existing `deltaKind` analysis — it applies to any direct TextRef read on a `value` attribute, regardless of whether `editText` is used. The write direction (`editText`) is a plain runtime function that translates `beforeinput` events into CRDT operations. Neither requires the other, and both compose naturally with other props.
