...
> **Status:** All phases (1–7) and all PRs (1–5) are complete. Test count is 938. See `.plans/kinetic-ir-dissolution.md` for Phase 3 sub-plan.
...
### Task 6.2: Parameterize `textRegion` / `inputTextRegion` (optional) — SKIPPED
...
>
> **Complete.** All 938 tests pass. Task 6.2 skipped (no third variant planned).

**Type:** Docs + hygiene (no behavior changes)

- Documented `claimSlot` runtime/compile-time slot kind divergence (JSDoc + TECHNICAL.md)
- Skipped `createDeltaTextRegion` factory (Task 6.2) — only two instances, no third variant planned
- Added cross-reference comment in `generateEscapeHelper` → `html-constants.ts`
- Removed dead-code `_reportMismatch` from `hydrate.ts` (zero call sites)
- Added `getActiveSubscriptions(): ReadonlyMap` to `subscribe.ts`; exported from both testing re-export files; kept mutable `activeSubscriptions` for test cleanup
- TECHNICAL.md: added "Shared Predicate Functions" design decision, updated file structure descriptions, added `unknown` boundary rationale in Loro Bindings section, documented slot kind divergence in Trackability Invariant
- **Files:** `regions.ts`, `subscribe.ts`, `hydrate.ts`, `codegen/html.ts`, `testing/index.ts`, `testing/runtime.ts`, `TECHNICAL.md`
- **Validates:** all 938 tests pass
...
PR 4 (bindings + transform) ✅ DONE ───────────────┤
                                                    └─── PR 5 (cleanup + docs) ✅ DONE
```

All 5 PRs are complete.
...
| After Phase 6+7 (cleanup + docs) | 938 |
