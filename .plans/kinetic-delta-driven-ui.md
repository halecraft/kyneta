...
|----------|-------------------------|
| **Uncompiled Source** | Valid TypeScript with full LSP support |
...
* Source is valid TS (LSP works), but compilation required for execution
```
...
__listRegion(doc.items, {
  onInsert: (item, index) => {
    const _li = document.createElement("li")
    _li.textContent = item.text
    __insertAt(container, _li, index)
  },
  onDelete: (index) => {
    __removeAt(container, index)
  },
  // No onUpdate needed - Loro tells us exactly what happened
})
```
...
These types define the user-facing API (what LSP understands):
...
function div(builder: () => void): Element
function div(props: Props, builder: () => void): Element
function div(props: Props, ...children: Child[]): Element
function div(...children: Child[]): Element
```
...
(() => {
  ...
  __listRegion(_ul, doc.items, {
    ...
    // Delta handlers generated by compiler
  })
  return _ul
})()
```
...
(() => {
  ...
  __conditionalRegion(_marker, () => doc.isEmpty.get(), {
    then: () => {
      const _p = document.createElement("p")
...
**⚠️ Risk Mitigation**: This is the core innovation. After completing Phase 3 (IR), 
spike this phase early to validate delta-to-DOM approach works before investing in Phases 4-5.
If the list transform fundamentally doesn't work, we need to know immediately.
...
- 🔴 **Task 6.2**: Generate delta handlers
  - `onInsert(item, index)` — create element, insert at position
  - `onDelete(index)` — remove element at position
  - `onMove(from, to)` — reorder DOM nodes (for MovableList)
  - Handle keyed items (if `item.id` exists)
...
packages/kinetic/
├── src/
│   ├── index.ts              # Public API (element functions, bind, mount)
...
│   └── types.ts              # User-facing type definitions
├── tests/
│   ├── runtime/
│   │   ├── subscribe.test.ts
│   │   ├── regions.test.ts
...
The list transform (Phase 6) is the core innovation. After Phase 3 (IR):

1. Spike Phase 6 to validate delta-to-DOM approach
2. Create minimal test: Loro list mutation → DOM update
3. Verify O(k) behavior with counting DOM
4. If fundamentals don't work, we know before investing in Phases 4-5

This de-risks the project by validating the key hypothesis early.
