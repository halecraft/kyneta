...
- **`ephemeral` is now a field-level CvRDT; the whole-document LWW substrate behind it is gone** (`@kyneta/schema`). The target keeps its name and its law set, so `ephemeral.bind(schema)` still compiles unchanged — but what it does underneath is different. Previously one timestamp covered the whole document, and the newest snapshot replaced all of it: two peers writing *different* fields meant whichever wrote last clobbered the other. That made the motivating use case — a presence roster where each peer writes only its own key — unusable. Now every scalar leaf carries its own `[value, timestamp]`, and concurrent writes to distinct fields both survive.

  Consequences worth knowing before upgrading:

  - **No stale rejection.** The old substrate had a total-order version, so an offer could be discarded as behind. `StateVersion.compare` reports `"concurrent"` for *every* comparison, because a payload that is older overall may still carry the newest value for some individual field. Every offer is therefore merged; the substrate is chattier in exchange for converging correctly. Note this extends to identical timestamps: reporting `"equal"` would make two peers who wrote to different fields in the same millisecond each discard the payload that would have reconciled them.
  - **Nothing survives a restart.** The old substrate could be persisted through a store, despite being declared `durability: "transient"`. It no longer can, which is what `transient` was always supposed to mean — a restarted server should not resurrect yesterday's cursor positions.
  - **The wire tag changed** from `["plain", 1, 0]` to `["ephemeral", 1, 0]`, so a 2.x peer and a 3.0 peer cannot sync an ephemeral document. Nothing is persisted, so there is nothing to migrate; a mixed deployment simply will not exchange presence until both sides are upgraded.

  **Removed with it:** `TimestampVersion` (from both `@kyneta/schema` and its `@kyneta/exchange` re-export) and the `buildUpgrade` barrel export, which existed only to be shared with the deleted substrate and had no remaining consumer.

- **`.decay()` below an opaque boundary is now rejected at `bind()` time** (`@kyneta/schema`). A schema that previously bound will now throw — but it never worked. Decay compares one stored timestamp against `now`, and a `sum` variant or `.json()` blob is stored as a single register tuple, so a field inside one has no timestamp of its own and could never age out independently. The binding was accepted and the decay silently never fired. **Migration:** move `.decay()` onto the sum or `.json()` node itself, which is supported and decays the whole value together; the error message says so. `.decay()` on ordinary fields is unaffected. The validator is renamed `validateSyncModeConstraints` → `validateDecayConstraints`, since it now enforces two rules and only one of them concerns sync mode; it is not part of the public API.

- **`advanceSchema`, `findJsonBoundary` and `JsonBoundaryHit` are removed** (`@kyneta/schema`). All three had zero production callers. Replace `advanceSchema(schema, segment)` with `walkPath`, which is public and is what every traversal in the package already uses — a caller who wants a single step wants a traversal. `findJsonBoundary` and `JsonBoundaryHit` were deprecated aliases; use `findOpaqueBoundary` and `OpaqueBoundaryHit`, which describe what they actually report (`.json()` nodes **and** sums). `advanceSchema` had been retained through 2.x specifically because it was public, but an exported single-step schema descent is what `packages/schema/TECHNICAL.md` §"Why one traversal, not many" identifies as the reason a previous traversal consolidation decayed into four divergent walkers.

- **The ephemeral `StateTuple` gains a third slot** (`@kyneta/schema`) — `[value, timestamp]` becomes `[value, timestamp, deleted?]`, where the third element marks a tombstone. A 3-element array serialises and parses with no wire-format change, so nothing on the transport is affected, but a 2.x peer cannot read it: the old `isStateTuple` rejected a tuple on arity, so it would be treated as a container and projected as `{0: value, 1: timestamp, 2: true}`. Mixed-version ephemeral sync is out of contract in 3.0.0. The target is `SYNC_EPHEMERAL`, so nothing persisted needs migrating.

## Added

- **Map deletes converge on the `ephemeral` substrate.** `mergeStateTree` unions keys, so absence carries no information — a key one peer lacks is indistinguishable from one it has never seen, and a removal survived only until the next merge with anyone who still held it. A `Schema.record` used as a roster could gain members but never lose them. A delete now writes a tombstone, which is an ordinary value that wins or loses by the normal rule, so the merge stays schema-blind for headless relays. Semantics are **LWW-Element-Set, not OR-Set**: concurrent add and remove resolve by timestamp, so a later add beats an earlier delete — for presence, a peer removed and rejoining should be present again. Deleting an entry whose value is a container tombstones the leaves inside it rather than replacing the subtree, which keeps every node's shape stable and so keeps the merge associative. Tombstones accumulate per *key*, not per operation — deleting replaces a tuple rather than adding one — so no collection mechanism is needed or provided. See `packages/schema/TECHNICAL.md` §"Deletion".

## Fixed

- **Transient documents no longer reach durable storage at all** (`@kyneta/exchange`). A document declared `durability: "transient"` — anything bound through `ephemeral` — used to get exactly one durable write, at creation: `Runtime` found nothing in the stores, registered the document, and an empty creation-time snapshot was written to every configured store. It then never updated, because a snapshot-only substrate returns `null` from `exportSince`, and it was faithfully hydrated on restart. The worst of both policies — a stale empty record on disk, plus a store round-trip on every open for data that could not be there, plus a delete on every teardown for something never stored.

  The exchange now reads the declared `durability` axis at every storage decision: no hydrate read, no `register`, no `state-advanced` write, no `destroy` delete. This makes the Breaking note above ("Nothing survives a restart") true by rule rather than by accident — previously it held only because the substrate happened not to support delta export, and would have reversed silently if it ever gained it.

  Two smaller consequences. Transient documents are now ready in the same tick rather than after a store round-trip. And `Runtime.#persistIfAdvanced` no longer runs its version-serialise, version-parse and `exportSince` work on every presence mutation before discovering there is nothing to write — for cursor movement, that was the most frequently executed wasted work in the system.

- **`needsContainer` now classifies `richtext` as a container** (`@kyneta/schema`), which it always was — a rich-text node is its own CRDT type, exactly as `text` is. It fell through the predicate's `switch` to a `default` of `false`, which broke the relation the eager-creation policies promise: `"leaf-containers"` (Yjs) is meant to be a strict subset of `"all-containers"` (Loro), and Loro was skipping a rich-text container that Yjs created. No user-visible behaviour changes — Loro creates the container lazily on first write and peers converge either way — but the two policies now nest as documented. `materializeValue` also gained the structural-zero default for rich text that `text` and `counter` already had, without which the newly-eager path produces a node the backends cannot build a container from. Both predicates now derive from a single `storageClass` classification rather than from separate switches that had to be kept in step by review.

- **Schema path-mismatch errors now name `stepSchema` rather than `advanceSchema`.** The twelve failure messages (`product has no field "x"`, `cannot advance through a sum`, and so on) are produced by `stepSchema` and surfaced as `walkPath`'s mismatch reason, which `foldPath` throws. They previously named a wrapper that no traversal called, so anyone who grepped the function in the error text found nothing relevant. Only the prefix changed; the descriptive text of each message is unchanged.

- **`Schema.record` is usable on the `ephemeral` substrate.** Every map write threw `TypeError: Cannot convert undefined or null to object`. `applyChangeToStateTree` read a `MapChange` shape the change vocabulary has never defined — per-key `{type: "set" | "delete"}` instructions under an `.entries` property — where the real change carries `set` (a key→value object) and `delete` (an array of keys). Both call sites, root and nested, carried the same mistake, and no test covered a record key write on this substrate, so it survived. The two sites are now one shared helper. This unblocks the presence-roster shape the substrate exists for: a record keyed by peer, each peer writing only its own key. Note that a `delete` is still a local removal only — it does not survive a merge with a peer that has not seen it. See below.

- **A map change aimed at an atomic register now throws instead of silently decomposing it.** A `sum` or `.json()` node is stored as one tuple so a concurrent variant switch resolves whole. The nested map path responded to finding one by overwriting it with `{}` and writing entries into it, splitting the register into blendable per-field tuples. `state.ts:prepare` widens such writes into a whole-value replace, so this is unreachable in normal operation — but the old behaviour was invisible when it did happen, because local reads are served from a separate shadow and only replicated state was damaged.

- **Two ephemeral peers writing in the same millisecond no longer diverge permanently.** `mergeStateTree` resolved a timestamp tie by taking the remote value, which is deterministic but not *commutative* — the property a CvRDT actually depends on. Merging A into B and B into A gave different answers, so each peer kept its own value and neither ever converged, with no error raised. Timestamps come from `Date.now()`, so a tie just means two peers wrote within the same millisecond; presence traffic, which arrives in bursts from many peers at once, is exactly the workload that produces them. Ties are now broken on the greater `JSON.stringify(value)`, giving a total order that every peer agrees on regardless of merge direction. Commutativity, associativity and idempotence are pinned as laws in `ephemeral-lattice.test.ts`. Which value survives a tie may change from previous releases; that it is the *same* value on both peers is the fix.
