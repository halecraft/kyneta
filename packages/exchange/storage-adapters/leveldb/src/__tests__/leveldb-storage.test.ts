...
//
// Runs the reusable StorageBackend conformance suite against
// LevelDBStorageBackend, plus LevelDB-specific tests for
...
import {
  ...
  collectAll,
  plainMetadata,
} from "@kyneta/exchange/src/testing/storage-backend-conformance.js"
import {
  LevelDBStorageBackend,
  encodeStorageEntry,
  decodeStorageEntry,
} from "../server.js"
import type { StorageEntry } from "@kyneta/exchange/src/storage/storage-backend.js"
...
describeStorageBackend(
  "LevelDBStorageBackend",
  () => new LevelDBStorageBackend(makeTmpDir()),
  async backend => {
...
describe("LevelDBStorageBackend — close + reopen", () => {
  it("data persists across close and reopen on the same path", async () => {
    ...
    const backend2 = new LevelDBStorageBackend(dir)
...
describe("encodeStorageEntry / decodeStorageEntry", () => {
  it("round-trips a JSON string payload (entirety)", () => {
    const entry: StorageEntry = {
      payload: { kind: "entirety", encoding: "json", data: '{"hello":"world"}' },
      version: "42",
    }
    const decoded = decodeStorageEntry(encodeStorageEntry(entry))
