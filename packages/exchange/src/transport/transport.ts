...
import type { TransportType, ChannelId, PeerIdentityDetails } from "../types.js"
...
type AdapterParams = {
   ...
   * If not provided, auto-generated as `{transportType}-{counter}`.
   */
  transportId?: string
}
...
type AdapterCallbacks = Omit<TransportContext, "identity">
...
export abstract class Transport<G> {
  ...
  _initialize(context: TransportContext): void {
    ...
    if (
      ...
      throw new Error(`Adapter ${this.transportType} already initialized`)
    }
  ...
  async _start(): Promise<void> {
    if (this.#lifecycle.state !== "initialized") {
      throw new Error(
        `Cannot start adapter ${this.transportType} in state ${this.#lifecycle.state}`,
      )
    }
