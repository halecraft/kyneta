...
> 5. **Runtime hooks bridge local→network:** The Runtime fires `onDocReady`, `onDocChangeset`, `onDocDestroyed`, `onDocSuspended`, `onDocResumed`. The Exchange wires these into the Synchronizer. A standalone Runtime (no Exchange) leaves them unset — docs work fully without a network.
...
### Writer restarts and epoch

`Line`'s outbox/inbox documents are ordinary `json.bind()` (`SYNC_AUTHORITATIVE`) docs, so they're backed by `PlainVersion` — which carries an `epoch` (see `@kyneta/schema`'s [`PlainVersion`](../schema/TECHNICAL.md#plainversion); `epoch` is a universal `Version` property, not Plain-specific). A writer that restarts with no persisted store (e.g. a browser refresh with the same `peerId`) mints a fresh epoch. `Line` reads this via the substrate-agnostic `version(doc).epoch` — no knowledge of `PlainVersion` or any concrete substrate, and no `as any` casts.

`Line`'s own envelope schema carries its sequence cursor paired with the epoch it was computed against: `ackSeq: number` + `ackEpoch: string` (replacing the bare `ack: number`, and renamed from `ackIncarnation`). This is necessary because `Line`'s hand-rolled `seq`/`ack` counters are well-ordered *only within one epoch* of the doc they're stamped on — exactly the same category error `PlainVersion` itself avoids by carrying epoch alongside value. Both comparison sites that trusted a bare counter — `#processInbox`'s dedup guard (`msg.seq <= #lastProcessedSeq`) and `#pruneOutbox`'s prune guard (`msg.seq <= remoteAck`) — detect a peer-epoch mismatch and reset safely: the receiver's `#lastProcessedSeq` resets to `0` when the inbox's epoch changes (a fresh lineage has no history to have "already processed"), and the sender's `#pruneOutbox` refuses to prune anything unless `ackEpoch` matches its own outbox's current epoch (an ack from before the sender's own restart cannot certify anything about messages minted after it). The `""` structural default for `ackEpoch` is the "never acked" sentinel — real epochs are always non-empty. This fix is entirely contained within `Line` and `packages/exchange/src/line.ts`; the sync layer above needs no changes.
...
| Application payload | User supplies `send` / `recv` schemas. The envelope (`seq`, `ackSeq`, `ackEpoch`, `nextSeq`) is this package's concern. |
...
Epoch-boundary detection is now driven by an explicit epoch comparison, not solely by payload shape: `#executeImportDocData` compares the incoming version's `Version.epoch` against the local replica's `version().epoch` (via the pure, unit-tested `isEpochBoundaryOffer(localEpoch, remoteEpoch)` classifier). `LEGACY_EPOCH` and `DEFAULT_EPOCH` on either side are excluded — neither is a trustworthy mismatch signal (LEGACY predates epoch support; DEFAULT is the normal lazy-mint/first-sync path, already handled correctly by `merge()`). This check fires independent of `payload.kind`, which is what lets it detect an identity-discontinuity reset even when the offending offer happens to carry a `"since"` payload rather than an `"entirety"`.

A second, independent trigger remains and is preserved as the `isEntirety && hasEverSynced` heuristic: CRDT state grows monotonically, and eventually peers compact — discarding history ops and snapshotting current state. A post-compaction `exportSince(oldVersion)` may return an entirety payload rather than a delta, because the substrate no longer retains the history needed to compute the delta. This is a same-epoch history gap, not an identity discontinuity — the explicit epoch check above cannot detect it (there is no epoch mismatch to find), so the heuristic remains the *only* signal for this case and is evaluated unconditionally alongside the epoch check (`isEpochBoundary || isLegacyReset`), not superseded by it.
...
1. **Accept the reset.** Discard local state, adopt the incoming entirety/lineage. For interpret-mode substrates, this calls `Substrate.resetFromEntirety(payload, remoteVersion, options)` — a dedicated method, decoupled from the routine `merge()` path (which now assumes shared causal ancestry and never adopts across epochs on its own). For replicate-mode substrates (headless replicas), `ReplicaFactory.fromEntirety()` replaces the whole replica, except for `state` (CvRDT), which falls through to `merge()` since field-level LWW absorption is always safe regardless of epoch boundary.
...
For durability guarantees, use the `cohort` predicate to prevent compaction past critical peers — this is strictly better than receiver-side rejection, which causes permanent divergence with no built-in reconciliation path. The cohort prevents the situation from arising: `Exchange.leastCommonVersion(docId)` computes the LCV over cohort members only, so `compact()` never advances past a cohort member's confirmed version. The default cohort (no policy) includes all synced peers, preserving backward compatibility. The cohort predicate does not help with the epoch-mismatch trigger — a writer restart with no persisted store is an identity discontinuity, not a version the cohort could have been ahead of.

**Plain lineage vectors (jj:kxswmuzx).** `PlainVersion` is now a single-entry version vector with genesis (`DEFAULT_EPOCH`) as the empty vector ⊥, so a **fresh peer joining an incumbent is a VV subset (`behind`) and syncs via the ordinary `merge()` path — not a reset**, and two independently-created fresh peers are `equal` (no false lineage rivalry). The reset/`canReset` machinery therefore fires only for a genuine identity discontinuity between two *authored* REAL lineages, where it resets receiver→sender **unconditionally**. That direction is deliberate and correct for the supported case: a serialized writer restarting with no persisted store mints a *fresh, low-value* lineage that must supersede the peer's *stale, higher-value* one — and **no version-intrinsic winner** (by value or key) could pick it, because the restart forgot its history and carries the *lower* counter (a monotonic generation can't rescue this either — it cannot survive a no-store restart to be incremented). The only case a deterministic winner could serve — two peers *symmetrically* authoring the same authoritative doc — is misuse of the single-writer (`writerModel: "serialized"`) model, and resolving it would require an authority/lineage-provenance signal the version does not carry. `canReset` remains the per-peer permission veto.

### What an epoch boundary is NOT

- **Not a protocol message.** There is no `reset` opcode. The decision is derived from an explicit `Version.epoch` comparison and/or `offer { payload: { kind: "entirety" } }` + existing local state.
- **Not inferred from payload shape alone.** The epoch is an explicit property of the payload and the version — the receiver compares epochs directly, independent of whether the payload happens to be an entirety or a since-delta.
