...
function assertFactsEqual(
  ...
  for (const key of batchKeys) {
    expect(
      incKeys.has(key),
      `${prefix}batch fact missing from incremental: ${key}`,
    ).toBe(true)
  }
  ...
  for (const key of incKeys) {
    expect(
      batchKeys.has(key),
      `${prefix}incremental fact missing from batch: ${key}`,
    ).toBe(true)
  }
...
describe("IncrementalProjection", () => {
  ...
  describe("orphan resolution", () => {
    ...
    it("orphan resolved when concurrent map child creates the slot", () => {
      ...
      const _childBob = makeMapChild("bob", 0, root.id, "name")
  ...
  describe("seq structure", () => {
    it("emits active_structure_seq + constraint_peer facts", () => {
      ...
      expect(seq?.fact.values[ACTIVE_STRUCTURE_SEQ.PARENT]).toBe(
        cnIdKey(root.id),
      )
    ...
    it("seq with origin references", () => {
      ...
      const seq = findFactByPredicate(delta, ACTIVE_STRUCTURE_SEQ.predicate)
      expect(seq).toBeDefined()
      expect(seq?.fact.values[ACTIVE_STRUCTURE_SEQ.ORIGIN_LEFT]).toBe(
        cnIdKey(e1.id),
      )
      expect(seq?.fact.values[ACTIVE_STRUCTURE_SEQ.ORIGIN_RIGHT]).toBe(null)
    })
  ...
  describe("current() accumulation", () => {
    it("accumulates value facts and seq facts together", () => {
      ...
      expect(predicates.filter(p => p === ACTIVE_VALUE.predicate).length).toBe(
        2,
      )
      expect(
        predicates.filter(p => p === ACTIVE_STRUCTURE_SEQ.predicate).length,
      ).toBe(1)
      expect(
        predicates.filter(p => p === CONSTRAINT_PEER.predicate).length,
      ).toBe(1)
    })
  })
