...
import type { RealityDelta } from "../../../src/kernel/incremental/types.js"
import { solve } from "../../../src/kernel/pipeline.js"
import { STUB_SIGNATURE } from "../../../src/kernel/signature.js"
import { allConstraints } from "../../../src/kernel/store.js"
import type {
  Constraint,
  Reality,
  RealityNode,
...
describe("IncrementalPipeline", () => {
  ...
  describe("bootstrap warm-start", () => {
    ...
    it("pipeline config matches bootstrap config", () => {
      ...
      expect(pipeline.config.retractionConfig).toEqual(
        result.config.retractionConfig,
      )
    })
  ...
  describe("differential equivalence: simple map", () => {
    ...
    it("nested map containers match batch", () => {
      ...
      const { constraint: profileC, id: profileId } = produceMapChild(
        agent,
        rootId,
        "profile",
      )
      ...
      const { constraint: nameC, id: nameId } = produceMapChild(
        agent,
        profileId,
        "name",
      )
  ...
  describe("differential equivalence: sequence", () => {
    it("sequential elements match batch at every step", () => {
      ...
      // Insert A → B → C
      const { constraint: e1C, id: e1Id } = produceSeqChild(
        agent,
        rootId,
        null,
        null,
      ...
      const { constraint: e2C, id: e2Id } = produceSeqChild(
        agent,
        rootId,
        e1Id,
        null,
      ...
      const { constraint: e3C, id: e3Id } = produceSeqChild(
        agent,
        rootId,
        e2Id,
        null,
  ...
  describe("differential equivalence: mixed containers", () => {
    it("map with nested seq matches batch", () => {
      ...
      // Seq child: todos
      const { constraint: todosC, id: _todosId } = produceMapChild(
        agent,
        rootId,
        "todos",
      )
    ...
    it("reality with both map and seq containers matches batch", () => {
      ...
      // Map container
      const { constraint: profileC, id: profileId } = produceRoot(
        agent,
        "profile",
        "map",
      )
      ...
      // Seq container
      const { constraint: todosC, id: todosId } = produceRoot(
        agent,
        "todos",
        "seq",
      )
      ...
      const { constraint: t1C, id: t1Id } = produceSeqChild(
        agent,
        todosId,
        null,
        null,
      ...
      const { constraint: t2C, id: t2Id } = produceSeqChild(
        agent,
        todosId,
        t1Id,
        null,
  ...
  describe("retraction cascade", () => {
    it("insert value, retract it — value disappears and matches batch", () => {
      ...
      const _retractDelta = insertAndVerify(pipeline, retractC, "retract")

      // Value should be gone (undefined)
      const _titleNode = getNode(pipeline.current(), "doc", "title")
  ...
  describe("authority change cascade", () => {
    ...
    it("revoke capability — peer constraints become invalid", () => {
      ...
      const revokeBob = alice.produceAuthority("bob", "revoke", {
        kind: "admin",
      })
  ...
  describe("multi-agent sync", () => {
    it("two pipelines converge after bidirectional constraint exchange", () => {
      ...
      const _aliceReality = alicePipeline.current()
      const _bobReality = bobPipeline.current()
      ...
      expect(
        realitiesEqual(alicePipeline.current(), bobPipeline.current()),
      ).toBe(true)
      ...
      // Verify content
      expect(getNode(alicePipeline.current(), "doc", "title")?.value).toBe(
        "Alice Title",
      )
      expect(getNode(alicePipeline.current(), "doc", "body")?.value).toBe(
        "Bob Body",
      )
    })
  ...
  describe("insertMany", () => {
    it("batch insert produces same reality as sequential inserts", () => {
      ...
      const { constraint: keyC2, id: keyId2 } = produceMapChild(
        agent2,
        rootId2,
        "key",
      )
  ...
  describe("retraction multi-removal fix (8.1a)", () => {
    it("multiple active constraints removed in same delta are all processed", () => {
      ...
      const _addResult = retraction.step(addDelta)
  ...
  describe("integration scenario replay", () => {
    it("simple map scenario from integration tests", () => {
      ...
      expect(getNode(pipeline.current(), "profile", "name")?.value).toBe(
        "Alice",
      )
    })
    ...
    it("three-agent sync scenario from integration tests", () => {
      ...
      const grantCharlie = alice.produceAuthority("charlie", "grant", {
        kind: "admin",
      })
      ...
      // Alice writes
      const { constraint: aKeyC, id: aKeyId } = produceMapChild(
        alice,
        rootId,
        "alice_key",
      )
      ...
      const bob = createAgent("bob")
      bob.observeMany(allConstraints(bobPipeline.store))
      const { constraint: bKeyC, id: bKeyId } = produceMapChild(
        bob,
        rootId,
        "bob_key",
      )
      ...
      const { constraint: cKeyC, id: cKeyId } = produceMapChild(
        charlie,
        rootId,
        "charlie_key",
      )
  ...
  describe("edge cases", () => {
    ...
    it("bookmark constraints pass through without affecting reality", () => {
      ...
      // Insert a bookmark
      const bookmark = agent.produceBookmark(
        "snapshot1",
        new Map([["alice", 100]]),
      )
    ...
    it("concurrent seq inserts by different peers converge", () => {
      ...
      const { constraint: aliceElemC, id: aliceElemId } = produceSeqChild(
        alice,
        rootId,
        null,
        null,
      ...
      const { constraint: bobElemC, id: bobElemId } = produceSeqChild(
        bob,
        rootId,
        null,
        null,
