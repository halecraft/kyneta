...
function batchActiveKeys(constraints: Constraint[], config = DEFAULT_RETRACTION_CONFIG): string[] {
  return computeActive(constraints, config).active.map((c) => cnIdKey(c.id)).sort();
}
...
describe('IncrementalRetraction', () => {
  ...
  describe('undo (retract of retract)', () => {
    it('original target re-activates', () => {
      ...
      expect(deltaActiveKeys(delta)).toEqual([cnIdKey(r2.id), cnIdKey(v1.id)].sort());
  ...
  describe('depth limits', () => {
    ...
    it('default config has maxDepth 2', () => {
      ...
      expect(activeKeys(stage)).toEqual([cnIdKey(r2.id), cnIdKey(r3.id), cnIdKey(v1.id)].sort());
    });
  });
  ...
  describe('target-in-refs validation', () => {
    ...
    it('retract with ref counter < target counter produces a violation', () => {
      ...
      const r1 = makeRetract('bob', 0, 2, createCnId('alice', 0), [
        createCnId('alice', 98),
      ]);
      ...
      expect(stage.violations().some((v) => v.reason.kind === 'targetNotInRefs')).toBe(true);
    });
  ...
  describe('out-of-order: multi-element delta', () => {
    it('retract + target in same delta: target is dominated', () => {
      ...
      const delta = stage.step(multiInsert([r1, v1]));
  ...
  describe('differential equivalence', () => {
    ...
    it('matches batch with depth limit 1', () => {
      ...
      expect(activeKeys(stage)).toEqual(batchActiveKeys([v1, r1, r2], depthConfig));
    });
