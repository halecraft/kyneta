...
function winnerZset(
  ...
  return zsetFromEntries(
    winners.map(
      w =>
        [w.slotId, { element: w, weight }] as [
          string,
          ZSetEntry<ResolvedWinner>,
        ],
    ),
  )
}
...
function pairZset(
  ...
  return zsetFromEntries(
    pairs.map(
      p =>
        [`${p.parentKey}|${p.a}|${p.b}`, { element: p, weight }] as [
          string,
          ZSetEntry<FugueBeforePair>,
        ],
    ),
  )
}

function _structureDelta(
  ...
  const delta = zsetFromEntries(
    structures.map(
      s =>
        [cnIdKey(s.id), { element: s as Constraint, weight: 1 }] as [
          string,
...
function _deltaKinds(rd: RealityDelta): string[] {
...
function createScaffold(): TestScaffold {
  ...
  function stepFull(
    ...
    return skeleton.step(winnerZset(winners), pairZset(pairs), indexDelta)
  }
...
describe("IncrementalSkeleton", () => {
  ...
  describe("map container + value", () => {
    it("root + map child + value: childAdded + valueChanged", () => {
      const { stepFull: _stepFull, skeleton, addStructures } = createScaffold()
      const root = makeRoot("alice", 0, "profile")
      const child = makeMapChild("alice", 1, root.id, "name")
      const _sid = slotId(child)
  ...
  describe("map child visibility", () => {
    ...
    it("value restored after retraction updates value via valueChanged", () => {
      ...
      expect(getNode(skeleton.current(), "profile", "name")).toBeDefined()
      expect(
        getNode(skeleton.current(), "profile", "name")?.value,
      ).toBeUndefined()
  ...
  describe("differential equivalence", () => {
    it("single root matches batch buildSkeleton", () => {
      const { stepFull, skeleton, structIndex: _structIndex } = createScaffold()
      const root = makeRoot("alice", 0, "profile")
  ...
  describe("complex scenarios", () => {
    it("mixed map and seq with nested structure", () => {
      ...
      const _items = makeMapChild("alice", 2, doc.id, "items")
      ...
      stepFull([tw, w1, w2], [pair], [doc, title, listRoot, e1, e2])
  ...
  describe("NodeDelta correctness", () => {
    ...
    it("valueChanged has correct path for nested node", () => {
      const { stepFull, skeleton: _skeleton } = createScaffold()
      const root = makeRoot("alice", 0, "doc")
