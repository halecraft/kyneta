...
function assertIndexEquals(
  ...
  // Compare slotGroups
  expect(actual.slotGroups.size, `${prefix}slotGroups.size`).toBe(
    expected.slotGroups.size,
  )
  ...
  expect(actual.structureToSlot.size, `${prefix}structureToSlot.size`).toBe(
    expected.structureToSlot.size,
  )
  for (const [key, sid] of expected.structureToSlot) {
    expect(
      actual.structureToSlot.get(key),
      `${prefix}structureToSlot[${key}]`,
    ).toBe(sid)
  }
  ...
  // Compare childrenOf
  expect(actual.childrenOf.size, `${prefix}childrenOf.size`).toBe(
    expected.childrenOf.size,
  )
  for (const [parentKey, expectedChildren] of expected.childrenOf) {
    ...
    expect(actualChildren?.size, `${prefix}childrenOf[${parentKey}].size`).toBe(
...
function assertSlotGroupEquals(
  ...
  expect(actual.structures.length, `${prefix}structures.length`).toBe(
    expected.structures.length,
  )
}
...
describe("IncrementalStructureIndex", () => {
  ...
  describe("differential equivalence", () => {
    ...
    it("root + seq children matches batch", () => {
      ...
      assertIndexEquals(
        stage.current(),
        buildStructureIndex([root, e1, e2, e3]),
      )
    })
    ...
    it("concurrent map children (multi-structure slot group) matches batch", () => {
      ...
      assertIndexEquals(
        stage.current(),
        buildStructureIndex([root, child1, child2]),
      )
    })
    ...
    it("non-structure constraints in delta do not affect equivalence", () => {
      ...
      assertIndexEquals(
        stage.current(),
        buildStructureIndex([root, value, child]),
      )
    })
  })
