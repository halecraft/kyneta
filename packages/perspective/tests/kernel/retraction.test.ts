...
import type { RetractionResult } from "../../src/kernel/retraction.js"
import {
  computeActive,
...
describe("computeActive", () => {
  ...
  describe("target-in-refs rule", () => {
    it("retract without any refs produces a violation", () => {
      ...
      const r1 = makeRetractConstraint(
        "alice",
        1,
        2,
        createCnId("alice", 0),
    ...
    it("Agent-produced retraction of non-frontier constraint succeeds", () => {
      ...
      const _hasLiteralTarget = retract.refs.some(
