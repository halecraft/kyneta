...
import { buildStructureIndex, type StructureIndex } from '../../src/kernel/structure-index.js';
import {
  nativeResolution,
...
describe('skeleton: map null-deletion', () => {
  ...
  it('map child with null value but nested children is still present', () => {
    ...
    const { structureIndex, active } = setup([root, address, city, valAddress, valCity]);
    const reality = buildSkeleton(structureIndex, active);
    ...
    expect(getNode(reality, 'profile', 'address', 'city')!.value).toBe('Springfield');
  });
...
describe('skeleton: slot group merging (concurrent map creation)', () => {
  it('two peers creating same map key are merged into one slot', () => {
    ...
    const { structureIndex, active } = setup([root, aliceName, bobName, aliceVal, bobVal]);
    const reality = buildSkeleton(structureIndex, active);
  ...
  it('slot group merging preserves children from both structures', () => {
    ...
    const { structureIndex, active } = setup([
      root, aliceSettings, bobSettings, theme, themeVal, lang, langVal,
    ]);
    const reality = buildSkeleton(structureIndex, active);
...
describe('skeleton: mixed nesting', () => {
  it('map inside map', () => {
    ...
    expect(getNode(reality, 'doc', 'address', 'city')!.value).toBe('Springfield');
  });
  ...
  it('seq inside map', () => {
    ...
    const { structureIndex, active } = setup([root, tagsStructure, e1, e2, v1, v2]);
    const reality = buildSkeleton(structureIndex, active);
  ...
  it('multiple root containers', () => {
    ...
    const { structureIndex, active } = setup([
      profile, settings, name, theme, valName, valTheme,
    ]);
    const reality = buildSkeleton(structureIndex, active);
  ...
  it('deeply nested maps (3 levels)', () => {
    ...
    expect(getNode(reality, 'doc', 'level1', 'level2', 'level3')!.value).toBe('deep');
  });
});
...
describe('skeleton: ResolutionResult path', () => {
  ...
  it('ResolutionResult with fugue_before pairs orders seq children', () => {
    ...
    const resolution = makeResolution(
      [
        { slotId: slotE1, winnerCnIdKey: cnIdKey(v1.id), content: 'alice-item' },
...
describe('skeleton: orphaned values', () => {
  it('value targeting unknown structure is ignored', () => {
    ...
    const orphanedVal = makeValue('alice', 2, 2, createCnId('alice', 99), 'Orphan');
    const realVal = makeValue('alice', 3, 3, child.id, 'Alice');

    const { structureIndex, active } = setup([root, child, orphanedVal, realVal]);
    const reality = buildSkeleton(structureIndex, active);
