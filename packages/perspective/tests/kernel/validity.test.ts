...
import { computeValid, filterValid } from "../../src/kernel/validity.js"
...
describe("computeValid", () => {
  ...
  describe("non-creator without capabilities", () => {
    ...
    it("non-creator structure constraint is invalid without capability", () => {
      const constraints: Constraint[] = [makeStructureConstraint("bob", 0, 1)]
  ...
  describe("bookmark constraints", () => {
    it("bookmarks require no capability (any peer can bookmark)", () => {
      const constraints: Constraint[] = [makeBookmarkConstraint("bob", 0, 1)]
  ...
  describe("auditability", () => {
    it("invalid constraints include the constraint and error details", () => {
      const bobValue = makeValueConstraint(
        "bob",
        0,
        1,
        createCnId("alice", 0),
  ...
  describe("determinism", () => {
    it("same S produces same Valid(S) regardless of order", () => {
      ...
      expect(result1.invalid.every(i => i.constraint.id.peer === "carol")).toBe(
        true,
      )
      expect(result2.invalid.every(i => i.constraint.id.peer === "carol")).toBe(
        true,
      )
      expect(result3.invalid.every(i => i.constraint.id.peer === "carol")).toBe(
        true,
      )
    })
  })
  ...
  describe("edge cases", () => {
    ...
    it("creator with no constraints still has Admin in authority state", () => {
      const result = computeValid([], "alice")
      expect(
        hasCapability(result.authorityState, "alice", { kind: "admin" }),
      ).toBe(true)
    })

    it("admin granted to non-creator gives them full access", () => {
      const constraints: Constraint[] = [
        makeAuthorityConstraint("alice", 0, 1, "bob", "grant", {
          kind: "admin",
        }),
        makeStructureConstraint("bob", 0, 2),
