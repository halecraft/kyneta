...
describe('IncrementalLWW', () => {
  describe('single value insertion', () => {
    it('first value becomes winner with +1 delta', () => {
      ...
      const delta = lww.step(zsetSingleton(facts[0]!.predicate + '|' + cnIdKey(val1.id), facts[0]!, 1));
  ...
  describe('superseding value', () => {
    it('emits +1 only for replacement (not −1 then +1)', () => {
      ...
      const d1 = lww.step(zsetSingleton(facts[0]!.predicate + '|' + cnIdKey(val1.id), facts[0]!, 1));
      expect(zsetSize(d1)).toBe(1);

      // Insert val2 (lamport 20) — supersedes val1
      const d2 = lww.step(zsetSingleton(facts[1]!.predicate + '|' + cnIdKey(val2.id), facts[1]!, 1));
  ...
  describe('non-superseding value', () => {
    it('emits empty delta when loser arrives', () => {
      ...
      lww.step(zsetSingleton(facts[0]!.predicate + '|' + cnIdKey(val2.id), facts[0]!, 1));

      // Insert val1 (lamport 10 — loses)
      const d2 = lww.step(zsetSingleton(facts[1]!.predicate + '|' + cnIdKey(val1.id), facts[1]!, 1));
  ...
  describe('value retraction', () => {
    it('retracting the winner promotes the next entry', () => {
      ...
      // Insert both
      lww.step(zsetSingleton(facts[0]!.predicate + '|' + cnIdKey(val1.id), facts[0]!, 1));
      lww.step(zsetSingleton(facts[1]!.predicate + '|' + cnIdKey(val2.id), facts[1]!, 1));

      // Winner is val2 (higher lamport)
      expect([...lww.current().values()][0]!.winnerCnIdKey).toBe(cnIdKey(val2.id));

      // Retract val2
      const d3 = lww.step(zsetSingleton(facts[1]!.predicate + '|' + cnIdKey(val2.id), facts[1]!, -1));
    ...
    it('retracting sole entry emits −1', () => {
      ...
      // Retract val1
      const d2 = lww.step(zsetSingleton(facts[0]!.predicate + '|' + cnIdKey(val1.id), facts[0]!, -1));
    ...
    it('retracting a non-winner emits empty delta', () => {
      ...
      const d3 = lww.step(zsetSingleton(facts[0]!.predicate + '|' + cnIdKey(val1.id), facts[0]!, -1));
  ...
  describe('multiple slots', () => {
    it('tracks slots independently', () => {
      ...
      lww.step(zsetSingleton(facts[0]!.predicate + '|' + cnIdKey(val1.id), facts[0]!, 1));
      lww.step(zsetSingleton(facts[1]!.predicate + '|' + cnIdKey(valBody.id), facts[1]!, 1));
  ...
  describe('ignores non-active_value facts', () => {
    it('skips facts with different predicates', () => {
      const lww = createIncrementalLWW();
      const otherFact = fact('active_structure_seq', ['cn1', 'parent1', null, null]);
  ...
  describe('peer tiebreak', () => {
    it('higher peer wins on lamport tie', () => {
      ...
      lww.step(zsetSingleton(facts[0]!.predicate + '|' + cnIdKey(val2.id), facts[0]!, 1));
      lww.step(zsetSingleton(facts[1]!.predicate + '|' + cnIdKey(val3.id), facts[1]!, 1));
