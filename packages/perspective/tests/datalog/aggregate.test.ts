...
describe("evaluateAggregation", () => {
  describe("count", () => {
    ...
    it("count always returns a number", () => {
      ...
      const db = dbFromFacts([fact("data", ["x", 1n]), fact("data", ["x", 2n])])
    ...
    it("count with a single group", () => {
      ...
      const db = dbFromFacts([fact("items", ["only", "a"])])
  ...
  describe("sum", () => {
    it("sums numbers", () => {
      ...
      const transport = results.find(
        s => s.bindings.get("Category") === "transport",
      )
  ...
  describe("max", () => {
    ...
    it("max with mixed number/bigint is a type error", () => {
      ...
      const db = dbFromFacts([fact("data", ["x", 3]), fact("data", ["x", 5n])])
    ...
    it("max with single element returns that element", () => {
      ...
      const db = dbFromFacts([fact("data", ["x", 42])])
  ...
  describe("min", () => {
    ...
    it("min with single element returns that element", () => {
      ...
      const db = dbFromFacts([fact("data", ["x", 77])])
...
describe("evaluateAggregationForSubs", () => {
  ...
  it("returns empty for empty substitution list", () => {
    ...
    const db = dbFromFacts([fact("data", ["x", 1])])
...
describe("multiple groupBy variables", () => {
  it("groups by two variables", () => {
    ...
    const xp = results.find(
      s => s.bindings.get("A") === "x" && s.bindings.get("B") === "p",
    )
    const xq = results.find(
      s => s.bindings.get("A") === "x" && s.bindings.get("B") === "q",
    )
    const yp = results.find(
      s => s.bindings.get("A") === "y" && s.bindings.get("B") === "p",
    )
...
describe("aggregation in full evaluation", () => {
  ...
  it("aggregation over empty derived relation produces no groups", () => {
    ...
    const countRule: Rule = rule(atom("result", [varTerm("G"), varTerm("C")]), [
      aggregation(aggClause),
    ])
...
describe("aggregation edge cases", () => {
  ...
  it("sum aggregation with all zeros", () => {
    ...
    const db = dbFromFacts([fact("data", ["x", 0]), fact("data", ["y", 0])])
