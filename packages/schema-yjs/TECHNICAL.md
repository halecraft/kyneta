# @kyneta/schema-yjs — Technical Documentation

## Architecture Overview

`@kyneta/schema-yjs` implements `Substrate<YjsVersion>` by wrapping a `Y.Doc` with schema-aware typed reads, writes, versioning, and export/import. The architecture mirrors `@kyneta/schema-loro` but is structurally simpler due to Yjs's imperative mutation model (no intermediate diff format, no synthetic container IDs).
...
```
packages/schema-yjs/
├── package.json          # @kyneta/schema-yjs, peerDeps on schema + yjs
