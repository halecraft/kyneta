...
119 tests across 4 files (consolidated from the plan's 7 files for higher signal-to-maintenance ratio). Schema constructor tests folded into `interpret.test.ts`, feed tests folded into `writable.test.ts`, action tests folded into `step.test.ts`.

- Task: `step.test.ts` (34 tests) — cursor semantics for stepText/stepSequence (retain/insert/delete combinations, empty input, passthrough), stepMap set/delete ordering, stepReplace, stepIncrement, generic `step()` dispatcher including unknown-type throw, multi-action folding. 🟢
- Task: `zero.test.ts` (28 tests) — `Zero.structural` for all scalar kinds, product/sequence/map/sum structural kinds, annotation defaults (text→"", counter→0, doc→delegates, movableList→[], unknown→undefined), nested realistic doc schema, `Zero.overlay` deep merge (per-key product recursion, sequence wholesale, nested 3-level deep), `Zero.for`/`Zero.partial` identity. 🟢
- Task: `interpret.test.ts` (19 tests) — catamorphism laziness (side-effect counter proves zero thunks forced after interpret, incremental forcing), zero equivalence (flat doc, nested products, discriminated sums, positional sums), plain round-trip (flat, nested with all kinds, sparse/missing data), schema constructor grammar verification (7 constructors → correct `_kind`/`tag`), enrich combinator (adds FEED, preserves product-level results), path accumulation tracking. 🟢
- Task: `writable.test.ts` (38 tests) — product lazy getters (referential identity, sibling independence), namespace isolation (Object.keys, FEED non-enumerable, hasOwn for non-schema keys), scalar upward write (.set→store, .get live read), portable refs (scalar and text extracted and used outside tree), text ref (get/toString/insert/delete/update), counter ref (get/increment/decrement/default-arg), sequence ref (length/get-child/push/delete), map Proxy (dynamic key access, Object.keys, 'in' operator, FEED via symbol), batched mode (pending accumulates, flush applies), auto-commit, feed subscription (head/head-after-mutation/subscribe+unsub lifecycle, isFeedable for products/text/counter), annotation-driven behavior (text has insert/delete, plain.string has get/set only, counter has increment/decrement). 🟢
