...
- Task: Add `ensureCacheWiring(ctx)` to `with-caching.ts` — idempotent `prepare` wrapping, returns a `Map<string, (change: ChangeBase) => void>` (the invalidation handler map). Uses `pathKey` from the shared module. Duck-types `ctx` to detect `prepare` — read-only stacks (`RefContext` without `prepare`) skip wiring, preserving backward compatibility. 🟢
- Task: In each composite case (product, sequence, map), register the existing `[INVALIDATE]` handler in the invalidation map via `ensureCacheWiring(ctx)`. The handler logic (field state clear, `planCacheUpdate` + `applyCacheOps`) is unchanged — it just lives in the prepare pipeline now instead of on the ref. `[INVALIDATE]` remains on refs as a public symbol for direct use. 🟢
- Task: **In the same commit**, remove the 8 `if (INVALIDATE in result) result[INVALIDATE](change)` lines from `withWritable` mutation methods (scalar `.set()`, product `.set()`, sequence `.push()`, `.insert()`, `.delete()`, map `.set()`, `.delete()`, `.clear()`). Each method simplifies to: construct the change, `ctx.dispatch(path, change)`. 🟢
- Task: Remove the now-unused `INVALIDATE` import from `writable.ts`. Update stale comments in `writable.ts` that reference invalidate-before-dispatch. 🟢
- Task: Update the `withWritable` header comment and `withCaching` header comment to document the pipeline model. (`TECHNICAL.md` updates deferred to Phase 7.) 🟢
- Task: Verify all existing tests pass — 797/797 schema, 869/869 core. 🟢

The `prepare` pipeline after this phase (showing nesting, not flat sequence):

```
ctx.prepare(path, change)
  → withChangefeed's wrapper (outermost — wrapped second):
      calls inner prepare:
        → withCaching's wrapper (inner — wrapped first):
            invalidate cache at path
            calls original prepare:
...
Then `ctx.flush(origin?)` delivers all accumulated notifications as batched Changesets.
...
**Stated invariant:** Interpreter transformers compose by wrapping `prepare`/`flush` in interpretation order. The wrapping order is determined by call order during `interpret()`: inner interpreters wrap first, outer interpreters wrap second (becoming the outermost wrapper).

The `prepare` pipeline after Phases 3–4 has hooks from both layers:

```
ctx.prepare(path, change)                          # withChangefeed's wrapper (outermost)
  → originalPrepare(path, change)                  #   calls inner = withCaching's wrapper
      → handler(change)  # invalidate cache        #     withCaching fires handler BEFORE forwarding
      → originalPrepare(path, change)              #     calls base = applyChangeToStore
          → applyChangeToStore(store, path, change)
  → pending.push({ path, change })                 #   withChangefeed accumulates AFTER forwarding
...
ctx.flush(origin?)
  → withChangefeed's hook: planNotifications (pure) → deliverNotifications (imperative)
    ...
    Clear accumulator before firing (re-entrancy safe).
    Call inner flush (base is no-op).
```

The hook installation order is determined by interpretation order. During `interpret()`, `withChangefeed.product(ctx)` calls `base.product(ctx)` which cascades down to `withCaching.product(ctx)`. `withCaching` calls `ensureCacheWiring(ctx)` first (wrapping `ctx.prepare`). When control returns to `withChangefeed`, it calls `ensurePrepareWiring(ctx)` second (wrapping around the caching wrapper). Since `withChangefeed`'s wrapper is outermost, it calls `originalPrepare` (the caching wrapper) first, then accumulates. The caching wrapper fires the invalidation handler, then calls its own `originalPrepare` (the base `applyChangeToStore`).

Effective per-change order: **invalidate cache → store mutation → accumulate notification**, then **plan + deliver notifications** once per batch. This ensures caches are invalidated before store mutation, and subscribers see fully-applied state when notifications deliver.
...
|---|---|---|
| `INVALIDATE` import | `writable.ts` L49–51 | ✅ Removed in PR 5 — all 8 usage sites removed in Phase 4 |
...
| stale comments | `writable.ts` header, withWritable JSDoc | ✅ Updated in PR 5 to describe prepare-pipeline model |
...
`withCaching` hooks into the `prepare` phase via `ensureCacheWiring`. Simultaneously, the 8 `if (INVALIDATE in result) result[INVALIDATE](change)` guards are removed from `withWritable` mutation methods.

- Add `ensureCacheWiring(ctx)` to `with-caching.ts` — duck-types `ctx` for `prepare` (read-only stacks skip wiring) 🟢
- Add `hasPrepare(ctx)` type guard and `registerCacheHandler` helper 🟢
- Register invalidation handlers by path in each composite case (product, sequence, map) 🟢
- Remove 8 INVALIDATE guards from `withWritable` mutation methods 🟢
- Remove unused `INVALIDATE` import from `writable.ts` 🟢
- Update stale comments in `writable.ts` and header comments in both files 🟢
- Verify all existing tests pass — 797/797 schema, 869/869 core 🟢
