// bind-constraints-state — what `state.bind()` accepts and rejects.
//
// `state` is a field-level LWW map. It declares a closed law set,
...
describe("state.bind() accepts LWW-family schemas", () => {
  it("scalars", () => {
  ...
  it("nested structs", () => {
    ...
    expect(state.bind(schema).schema).toBe(schema)
  })
...
describe("state.bind() rejects CRDT schemas", () => {
