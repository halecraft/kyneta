...
| `WireOpts` | `src/pipeline.ts` | Optional pipeline configuration (threshold, reassembly limits, `onError`, `onFrame`). |
...
- **Imperative shell**: `Pipeline` (in `pipeline.ts`) holds the `PipelineState` (alias table, reassembler, and the `nextSeq` frame-sequence counter) and delegates all logic to the step functions. `nextSeq` is *pure data* threaded through `PipelineState` and advanced functionally by `sendStep` (via `nextFrameSeq`) — not an injected closure — so `sendStep` stays a pure transition. The shell routes errors through `onError` and per-frame traces through `onFrame` (both opt-in, for observability).
