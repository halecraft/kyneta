...
describe('Agent', () => {
  ...
  describe('counter monotonicity', () => {
    it('produces constraints with monotonically increasing counters', () => {
      const agent = createAgent('alice');
      const c1 = agent.produceStructure({ kind: 'root', containerId: 'a', policy: 'map' });
      const c2 = agent.produceStructure({ kind: 'root', containerId: 'b', policy: 'map' });
      const c3 = agent.produceStructure({ kind: 'root', containerId: 'c', policy: 'map' });
    ...
    it('all produced constraints have the agent peerId', () => {
      const agent = createAgent('alice');
      const c = agent.produceStructure({ kind: 'root', containerId: 'test', policy: 'map' });
      expect(c.id.peer).toBe('alice');
    });

    it('counter starts from custom initial value', () => {
      const agent = createAgent('alice', STUB_PRIVATE_KEY, 100);
      const c = agent.produceStructure({ kind: 'root', containerId: 'a', policy: 'map' });
  ...
  describe('lamport monotonicity', () => {
    ...
    it('lamport values are strictly increasing across many produces', () => {
      ...
      for (let i = 0; i < 50; i++) {
        const c = agent.produceStructure({ kind: 'root', containerId: `c${i}`, policy: 'map' });
  ...
  describe('refs tracking', () => {
    ...
    it('third constraint refs only the latest per peer', () => {
      ...
      const c3 = agent.produceStructure({ kind: 'root', containerId: 'c', policy: 'map' }); // alice@2
  ...
  describe('produceStructure', () => {
    it('produces a structure constraint with correct fields', () => {
      const agent = createAgent('alice');
      const c = agent.produceStructure({ kind: 'root', containerId: 'profile', policy: 'map' });
  ...
  describe('produceRule', () => {
    it('produces a rule constraint with correct fields', () => {
      ...
      const body = [
        { kind: 'atom' as const, atom: { predicate: 'active_value', terms: [] } },
      ];
  ...
  describe('two-agent interaction', () => {
    it('two agents produce constraints with different CnIds', () => {
      ...
      const a1 = alice.produceStructure({ kind: 'root', containerId: 'a', policy: 'map' });
      const b1 = bob.produceStructure({ kind: 'root', containerId: 'b', policy: 'map' });
    ...
    it('agents that observe each other produce causally ordered constraints', () => {
      ...
      // Alice produces c1
      const c1 = alice.produceStructure({ kind: 'root', containerId: 'a', policy: 'map' });

      // Bob observes c1, then produces c2
      bob.observe(c1);
      const c2 = bob.produceStructure({ kind: 'root', containerId: 'b', policy: 'map' });
  ...
  describe('safe-integer overflow', () => {
    it('throws when counter would exceed MAX_SAFE_INTEGER', () => {
      ...
      const agent = createAgent('alice', STUB_PRIVATE_KEY, Number.MAX_SAFE_INTEGER + 1);

      expect(() => {
        agent.produceStructure({ kind: 'root', containerId: 'a', policy: 'map' });
      }).toThrow('counter overflow');
    });

    it('does not throw at MAX_SAFE_INTEGER counter', () => {
      // MAX_SAFE_INTEGER is safe, should not throw
      const agent = createAgent('alice', STUB_PRIVATE_KEY, Number.MAX_SAFE_INTEGER);
  ...
  describe('realistic scenario', () => {
    it('simulates a multi-agent editing session', () => {
      ...
      const { constraint: rootConstraint, id: rootId } = produceRoot(alice, 'doc', 'map');
      const { constraint: titleSlot, id: titleSlotId } = produceMapChild(alice, rootId, 'title');
      ...
      const { constraint: bodySlot, id: bodySlotId } = produceMapChild(alice, rootId, 'body');
