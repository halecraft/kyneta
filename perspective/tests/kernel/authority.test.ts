...
const CREATE_NODE_PROFILE: Capability = { kind: 'createNode', pathPattern: ['profile'] };
...
const AUTHORITY_WRITE_PROFILE: Capability = { kind: 'authority', capability: WRITE_PROFILE };
...
describe('capabilityEquals', () => {
  ...
  it('write with same path', () => {
    expect(capabilityEquals(WRITE_PROFILE, { kind: 'write', pathPattern: ['profile'] })).toBe(true);
  });
  ...
  it('retract scopes compared correctly', () => {
    ...
    const byPath1: Capability = { kind: 'retract', scope: { kind: 'byPath', pattern: ['a'] } };
    const byPath2: Capability = { kind: 'retract', scope: { kind: 'byPath', pattern: ['a'] } };
    const byPath3: Capability = { kind: 'retract', scope: { kind: 'byPath', pattern: ['b'] } };
...
describe('capabilityCovers', () => {
  ...
  it('authority(C) covers authority(C) for same C', () => {
    ...
    const required: Capability = { kind: 'authority', capability: WRITE_PROFILE };
...
describe('capability attenuation', () => {
  it('admin holder can grant anything', () => {
    ...
    expect(hasCapability(state, 'alice', { kind: 'authority', capability: WRITE_PROFILE })).toBe(true);
    expect(hasCapability(state, 'bob', WRITE_PROFILE)).toBe(true);
  });
...
describe('authority determinism', () => {
  it('same constraints produce same result regardless of insertion order', () => {
    const c1 = makeAuthorityConstraint('alice', 0, 1, 'bob', 'grant', WRITE_PROFILE);
    const c2 = makeAuthorityConstraint('alice', 1, 2, 'bob', 'grant', WRITE_TODOS);
    const c3 = makeAuthorityConstraint('carol', 0, 2, 'bob', 'revoke', WRITE_PROFILE);
