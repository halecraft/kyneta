...
import { resolveLWW, type LWWEntry } from '../../src/solver/lww.js';
...
function runNativeLWW(
  ...
  for (const [slot, winner] of result.winners) {
    winners.set(slot, { winnerId: cnIdKey(winner.winnerId), content: winner.content });
  }
  return winners;
}
...
describe('LWW equivalence: native == Datalog', () => {
  ...
  it('multiple independent slots — each resolved separately', () => {
    ...
    const all: Constraint[] = [root, title, body, titleVal1, titleVal2, bodyVal1, bodyVal2];
