...
describe('Fugue rules (simplified subset)', () => {
  ...
  it('concurrent inserts at same position: lower peer goes first', () => {
    ...
    expect(hasFact(db, 'fugue_before', ['root', 'elem_a', 'elem_c'])).toBe(
      true,
    );
  ...
  it('deterministic ordering: same inputs → same results regardless of fact order', () => {
    ...
    expect(hasFact(result1.value, 'fugue_before', ['root', 'y', 'z'])).toBe(true);
    expect(hasFact(result1.value, 'fugue_before', ['root', 'y', 'x'])).toBe(true);
    expect(hasFact(result1.value, 'fugue_before', ['root', 'z', 'x'])).toBe(true);

    expect(hasFact(result2.value, 'fugue_before', ['root', 'y', 'z'])).toBe(true);
    expect(hasFact(result2.value, 'fugue_before', ['root', 'y', 'x'])).toBe(true);
    expect(hasFact(result2.value, 'fugue_before', ['root', 'z', 'x'])).toBe(true);
