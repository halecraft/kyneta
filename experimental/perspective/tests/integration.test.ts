...
function bidirectionalSync(
  ...
  if (!resultAtoB.ok)
    throw new Error(`Sync A→B failed: ${JSON.stringify(resultAtoB.error)}`)

  // B → A
  const deltaBtoA = exportDelta(storeB, getVersionVector(storeA))
  const resultBtoA = importDelta(storeA, deltaBtoA)
  if (!resultBtoA.ok)
    throw new Error(`Sync B→A failed: ${JSON.stringify(resultBtoA.error)}`)
}
...
describe("bootstrap", () => {
  ...
  it("contains default LWW rules at Layer 1", () => {
    ...
    const lwwRules = ruleConstraints.filter(
      r =>
        r.payload.head.predicate === "superseded" ||
        r.payload.head.predicate === "winner",
    )
...
describe("two-agent sync", () => {
  it("bidirectional delta exchange produces convergent realities", () => {
    ...
    const bobDeltaFromAlice = exportDelta(
      aliceStore,
      getVersionVector(bobStore),
    )
  ...
  it("three-agent sync: all converge after pairwise exchange", () => {
    ...
    importDelta(
      charlieStore,
      exportDelta(aliceStore, getVersionVector(charlieStore)),
    )
...
describe("retraction sync", () => {
  ...
  it("retraction of seq value makes element a tombstone", () => {
    ...
    expect(getSeqValues(solve(aliceStore, config), "list")).toEqual([
      "A",
      "B",
      "C",
    ])
...
describe("multi-container", () => {
  ...
  it("nested containers: map-in-seq and seq-in-map", () => {
    ...
    const { constraint: itemsC, id: itemsId } = produceMapChild(
      alice,
      rootId,
      "items",
    )
    ...
    const { constraint: countC, id: countId } = produceMapChild(
      alice,
      rootId,
      "count",
    )
...
describe("constraint auditability", () => {
  ...
  it("authorized agent after grant can contribute to reality", () => {
    ...
    expect(
      result.validityResult.invalid.some(ic => ic.constraint.id.peer === "bob"),
    ).toBe(false)
    expect(getNode(result.reality, "doc", "field")?.value).toBe(
      "Bob-wrote-this",
    )
  })
