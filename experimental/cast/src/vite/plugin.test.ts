...
function getPlugin(options?: KynetaPluginOptions) {
...
describe("Vite Plugin", () => {
  ...
  describe("import handling", () => {
    it("should add runtime imports when needed for list regions", () => {
      ...
      expect(result?.code).toContain('@kyneta/cast/runtime"')
    })
    ...
    it("should merge imports with existing kyneta imports", () => {
      ...
      const source = `
        import { mount, Scope } from "@kyneta/cast"
      ...
      expect(result?.code).toContain('@kyneta/cast"')
    })
